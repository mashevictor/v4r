...
class Category {
 public:
  ...
  std::map<std::string, size_t>
      confusing_classes_;  // counts how many times this category is confused by another category
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < rel_files.size(); i++) {
    ...
    if (prediction.compare(ground_truth.str()) == 0) {
      num_correct++, classes[prediction].true_positives_++;
    } else {
      ...
      of << ground_truth.str() << " " << prediction_esf << " " << confidence_esf << " " << prediction_cnn << " "
         << confidence_cnn;
      of.close();
    }
  }
