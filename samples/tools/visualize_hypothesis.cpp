...
Eigen::Matrix4f readMatrixFromFile(const std::string &file, int padding) {
...
int main(int argc, char **argv) {
  ...
  po::options_description desc(
      "Visualize scene with object hypotheses in a given pose\n======================================\n**Required "
      "options");
  desc.add_options()("help,h", "produce help message")("scene_path,s", po::value<std::string>(&path_s)->required(),
                                                       "Filename of the point cloud of the scene")(
      "model_path,m", po::value<std::string>(&path_m)->required(),
      "Filename of the point cloud of the model (e.g. 3D_model.pcd)")(
      "model_pose,p", po::value<std::string>(&path_p)->required(),
      "Filename of the 4x4 object pose stored in row-major order");
  po::variables_map vm;
