...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help,h", "produce help message")(
      "input_training_dir,t", po::value<std::string>(&training_dir)->required(),
      "root directory containing directories with training views, pose and object mask of each object model "
      "(recognition structure)")("input_models_dir,m", po::value<std::string>(&model_dir),
      ...
      "training view basename prefix (e.g. cloud_, view_)")(
      "pose_prefix", po::value<std::string>(&pose_prefix)->default_value(pose_prefix), "camera pose basename prefix")(
      "indices_prefix", po::value<std::string>(&indices_prefix)->default_value(indices_prefix),
      "object indices basename prefix");
  ...
  for (const std::string &model_training_path : object_models) {
    ...
    std::vector<std::string> views =
        v4r::io::getFilesInDirectory(full_model_training_path, training_view_regex_pattern, false);
    ...
    if (views.empty()) {  // does it have classes and object identities?
      ...
      if (class_models.empty()) {
        std::cerr << "Cannot find any training views in " << full_model_training_path
                  << "! Is the training dir and view prefix set correctly?" << std::endl;
        continue;
      } else {
    ...
    std::vector<std::string> files_copied;  // save copied files to avoid copying redundant files in trained descriptors
                                            // (in old model database views were saved multiple times)
    ...
    for (const std::string &desc_folder : descriptors_folders) {
      ...
      std::vector<std::string> desc_files =
          v4r::io::getFilesInDirectory(full_model_training_path + desc_folder, ".*", false);
      ...
      for (const std::string desc_file : desc_files) {
        ...
        if (!file_already_copied) {
          bf::copy_file(full_model_training_path + desc_folder + "/" + desc_file,
                        out_model_dir + "/" + desc_folder + "/" + desc_file);
        }
      }
    }
  }
