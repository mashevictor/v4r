...
class V4R_EXPORTS ViewVolume {
 public:
  ...
  int computeVisible(const typename pcl::PointCloud<PointType>::Ptr input, std::vector<bool> &mask) {
...
class ViewportChecker {
 public:
  ...
  void getVisibles(const typename pcl::PointCloud<PointType>::Ptr input,
    ...
    for (typename std::vector<ViewVolume<PointType>>::iterator vol = volumes.begin(); vol < volumes.end(); vol++) {
