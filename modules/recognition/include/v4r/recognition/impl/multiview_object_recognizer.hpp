...
/**
 * @file multiview_object_recognizer.hpp
...
bool MultiviewRecognizer<PointT>::calcSiftFeatures(const typename pcl::PointCloud<PointT>::Ptr &cloud_src,
...
template <typename PointT>
void MultiviewRecognizer<PointT>::pruneGraph() {
...
template <typename PointT>
bool MultiviewRecognizer<PointT>::computeAbsolutePose(CamConnect &e, bool is_first_edge) {
...
template <typename PointT>
void MultiviewRecognizer<PointT>::recognize() {
  if (!rr_)
    throw std::runtime_error(
        "Single-View recognizer is not set. Please provide a recognizer to the multi-view recognition system!");
  ...
  if (!scene_ || scene_->width != 640 || scene_->height != 480)
    throw std::runtime_error(
        "Size of input cloud is not 640x480, which is the only resolution currently supported by the verification "
        "framework.");
      ...
      if (param_.scene_to_scene_) {
        ...
        std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> sift_transforms =
            Registration::FeatureBasedRegistration<PointT>::estimateViewTransformationBySIFT(
                *w.scene_, *v.scene_, w.sift_kp_indices_, v.sift_kp_indices_, w.sift_signatures_, v.sift_signatures_,
                param_.use_gc_s2s_);
      ...
      if (!transforms.empty()) {
        ...
        for (size_t trans_id = 0; trans_id < transforms.size(); trans_id++) {
          ...
          try {
            ...
            calcEdgeWeightAndRefineTf<PointT>(w.scene_, v.scene_, e_tmp.transformation_, e_tmp.edge_weight_,
                                              icp_refined_trans);

            e_tmp.transformation_ = icp_refined_trans;
            std::cout << "Edge weight is " << e_tmp.edge_weight_ << " for edge connecting vertex " << e_tmp.source_id_
                      << " and " << e_tmp.target_id_ << " by " << e_tmp.model_name_;
            ...
            std::cerr << "Something is wrong with the SIFT based camera pose estimation. Turning it off and using the "
                         "given camera poses only."
                      << std::endl;
            continue;
          }
        }
  ...
  if (true) {  // we have to do the correspondence grouping ourselve [Faeulhammer et al 2015, ICRA paper]
    ...
    for (const auto &w_m : views_) {  // merge feature correspondences from other views
      ...
      for (const auto &oh_remote_m : w.hypotheses_) {
        LocalObjectHypothesis<PointT> oh_remote = oh_remote_m.second;  // copy because we need to update correspondences
                                                                       // indices and don't change the original
                                                                       // information
        ...
        if (oh_local_m == local_obj_hypotheses_.end())  // no feature correspondences exist yet
          ...
          // check each new correspondence for duplicate in existing database. If there is a sufficient close keypoint
...
#pragma omp parallel for
          for (size_t c_id = 0; c_id < new_corr; c_id++) {
            ...
            for (const pcl::Correspondence &c_existing : oh_local.model_scene_corresp_) {
              ...
              if ((squaredDistModelKeypoints < param_.distance_same_keypoint_) &&
                  ...
                  (s_kp_normal_new.getNormalVector3fMap().dot(s_kp_normal_existing.getNormalVector3fMap()) >
                   param_.same_keypoint_dot_product_)) {
                is_kept[c_id] = false;
                break;
              }
            }
          ...
          for (auto &corr :
               oh_remote.model_scene_corresp_)  // add appropriate offset to correspondence index of the scene cloud
    ...
    //        for(size_t m_id=0; m_id<transforms_.size(); m_id++) // transform hypotheses back from global coordinate
    //        system to current viewport
      ...
      for (size_t i = 0; i < w.models_.size(); i++) {
        if (w.model_or_plane_is_verified_[i]) {
          ...
          if (param_.merge_close_hypotheses_) {  // check if similar object hypotheses exist
            Eigen::Matrix4f object_pose_in_v_co_system =
            ...
            for (size_t j = 0; j < v.models_.size(); j++) {
              ...
              if ((dist < param_.merge_close_hypotheses_dist_) && (rotx < angle_thresh_rad) &&
                  (roty < angle_thresh_rad) && (rotz < angle_thresh_rad)) {
...

  {
    ...
    for (const auto &w_m : views_) {
      ...
      if (param_.run_reconstruction_filter_) {
        reconstructionFiltering(original_clouds[view_id], normal_clouds[view_id], w.absolute_pose_, w_m.first);
    ...
    if (views_.size() > 1) {  // don't do this if there is only one view otherwise point cloud is not kept organized and
                              // multi-plane segmentation takes longer
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < ohs.size(); i++) {
    ...
    for (size_t cluster_id = 0; cluster_id < corresp_clusters.size(); cluster_id++)
      t_est.estimateRigidTransformation(*oh.model_->keypoints_, scene_keypoints_, corresp_clusters[cluster_id],
    ...
    if (param_.merge_close_hypotheses_) {
      ...
      for (size_t tf_id = 0; tf_id < new_transforms.size(); tf_id++) {
        ...
        t_est.estimateRigidTransformation(*oh.model_->keypoints_, scene_keypoints_, merged_corrs,
...
#pragma omp critical
    ...
    std::cout << "Merged " << corresp_clusters.size() << " clusters into " << new_transforms.size()
              << " clusters. Total correspondences: " << oh.model_scene_corresp_.size() << " " << oh.model_->id_
              << std::endl;
