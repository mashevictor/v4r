...
/**
 * @file multiview_object_recognizer_change_detection.hpp
 * @author Martin Velas (ivelas@fit.vutbr.cz), Thomas Faeulhammer (faeulhammer@acin.tuwien.ac.at)
 * @date Jan, 2016
...
: MultiviewRecognizer<PointT>(argc, argv), changing_scene(new Cloud) {
  ...
  desc.add_options()(
      "min_points_for_hyp_removal",
      po::value<int>(&param_.min_points_for_hyp_removal_)->default_value(param_.min_points_for_hyp_removal_),
      "Minimal overlap with removed points in order to remove hypothesis")(
      "tolerance_for_cloud_diff",
      po::value<float>(&param_.tolerance_for_cloud_diff_)->default_value(param_.tolerance_for_cloud_diff_),
      "Tolerance when difference of clouds is computed");
  po::variables_map vm;
...
template <typename PointT>
void MultiviewRecognizerWithChangeDetection<PointT>::initHVFilters() {
  ...
  for (rem_it = removed_points_cumulated_history_.begin(); rem_it != removed_points_cumulated_history_.end();
       rem_it++) {
...
template <typename PointT>
void MultiviewRecognizerWithChangeDetection<PointT>::findChangedPoints(Cloud observation_unposed, Eigen::Affine3f pose,
                                                                       Cloud &removed_points, Cloud &added_points) {
...
void MultiviewRecognizerWithChangeDetection<PointT>::reconstructionFiltering(
    typename pcl::PointCloud<PointT>::Ptr observation, pcl::PointCloud<pcl::Normal>::Ptr observation_normals,
...
template <typename PointT>
void MultiviewRecognizerWithChangeDetection<PointT>::setNan(pcl::Normal &normal) {
...
template <typename PointT>
void MultiviewRecognizerWithChangeDetection<PointT>::setNan(PointT &pt) {
  const float nan_point = std::numeric_limits<float>::quiet_NaN();
  pt.x = nan_point;
  pt.y = nan_point;
  pt.z = nan_point;
...
template <typename PointT>
std::vector<bool> MultiviewRecognizerWithChangeDetection<PointT>::getHypothesisInViewsMask(ModelTPtr model,
  ...
  for (std::vector<size_t>::iterator view_id = sorted_view_ids.begin(); view_id < sorted_view_ids.end(); view_id++) {
    ...
    int rem_support = v4r::ChangeDetector<PointT>::removalSupport(removed_points_history_[*view_id], model_aligned,
                                                                  param_.tolerance_for_cloud_diff_, 1.1)
                          ->size();
...
template <typename PointT>
void MultiviewRecognizerWithChangeDetection<PointT>::cleanupHVFilters() {
