...
class V4R_EXPORTS LocalRecognizerParameter {
 public:
  ...
  float max_descriptor_distance_;  ///< maximum distance of the descriptor in the respective norm (L1 or L2) to create a
                                   /// correspondence
  float correspondence_distance_weight_;  ///< weight factor for correspondences distances. This is done to favour
  ...
  float threshold_planar_;         ///< threshold ratio used for deciding if patch is planar. Ratio defined as largest
                                   /// eigenvalue to all others.

  // parameters for depth-discontinuity filter
  int filter_border_pts_;  ///< Filter keypoints at the boundary (value according to the edge types defined in
  ...
  bool train_on_individual_views_;  ///< if true, extracts features from each view of the object model. Otherwise will
                                    /// use the full 3d cloud

  LocalRecognizerParameter()
  : kdtree_splits_(512), kdtree_num_trees_(4), knn_(1), max_descriptor_distance_(std::numeric_limits<float>::max()),
    correspondence_distance_weight_(1.f), distance_metric_(1),
    max_keypoint_distance_z_(std::numeric_limits<float>::max()), filter_planar_(false), min_plane_size_(1000),
    planar_support_radius_(0.04f), threshold_planar_(0.02f), filter_border_pts_(7), boundary_width_(5),
    required_viewpoint_change_deg_(10.f), train_on_individual_views_(true) {}
 ...
 private:
  ...
  V4R_EXPORTS void serialize(Archive &ar, const unsigned int version) {
    (void)version;
    ar &BOOST_SERIALIZATION_NVP(kdtree_splits_) & BOOST_SERIALIZATION_NVP(kdtree_num_trees_) &
        BOOST_SERIALIZATION_NVP(knn_) & BOOST_SERIALIZATION_NVP(max_descriptor_distance_) &
        BOOST_SERIALIZATION_NVP(correspondence_distance_weight_) & BOOST_SERIALIZATION_NVP(distance_metric_) &
        BOOST_SERIALIZATION_NVP(max_keypoint_distance_z_) & BOOST_SERIALIZATION_NVP(filter_planar_) &
        BOOST_SERIALIZATION_NVP(min_plane_size_) & BOOST_SERIALIZATION_NVP(planar_support_radius_) &
        BOOST_SERIALIZATION_NVP(threshold_planar_) & BOOST_SERIALIZATION_NVP(filter_border_pts_) &
        BOOST_SERIALIZATION_NVP(boundary_width_) & BOOST_SERIALIZATION_NVP(required_viewpoint_change_deg_) &
...
class V4R_EXPORTS LocalFeatureMatcher {
 ...
 private:
   ...
   */
  void featureMatching(const std::vector<KeypointIndex> &kp_indices, const std::vector<FeatureDescriptor> &signatures,
 ...
 public:
  ...
  : param_(p), have_sift_estimator_(false), visualize_keypoints_(false) {}
  ...
  */
   ...
   */
  void getKeypointIndices(std::vector<KeypointIndex> &indices) const {
