...
class FittingSurface {
 public:
  ...
  struct Parameter {
    ...
    Parameter(double intW = 1.0, double intS = 0.000001, double intR = 0.0, double bndW = 1.0, double bndS = 0.000001,
              double bndR = 0.0, unsigned regU = 0, unsigned regV = 0)
    : interior_weight(intW), interior_smoothness(intS), interior_regularisation(intR), boundary_weight(bndW),
      boundary_smoothness(bndS), boundary_regularisation(bndR), regularisation_resU(regU), regularisation_resV(regV) {}
  };
   ...
   */
  static Eigen::Vector2d inverseMapping(const ON_NurbsSurface &nurbs, const Eigen::Vector3d &pt,
  ...
  static Eigen::Vector2d inverseMapping(const ON_NurbsSurface &nurbs, const Eigen::Vector3d &pt,
                                        const Eigen::Vector2d &hint, Eigen::Vector3d &p, int maxSteps, double accuracy,
                                        bool quiet);
   ...
   * \return closest point on surface in parametric domain.
   */
  static Eigen::Vector2d inverseMappingBoundary(const ON_NurbsSurface &nurbs, const Eigen::Vector3d &pt, double &error,
   ...
   * \return closest point on surface in parametric domain.
   */
  static Eigen::Vector2d inverseMappingBoundary(const ON_NurbsSurface &nurbs, const Eigen::Vector3d &pt, int side,
  ...
  static ON_NurbsSurface initNurbsPCA(int order, NurbsDataSurface *data,
  ...
  static ON_NurbsSurface initNurbsPCABoundingBox(int order, NurbsDataSurface *data,
                                                 Eigen::Vector3d z = Eigen::Vector3d(0.0, 0.0, 1.0));
 ...
 protected:
  ...
  virtual void addPointConstraint(const Eigen::Vector2d &params, const Eigen::Vector3d &point, double weight,
                                  unsigned &row);
