...
SurfaceHeightSaliencyMap::SurfaceHeightSaliencyMap() : BaseMap() {
  reset();
}
...
void SurfaceHeightSaliencyMap::calculateHeightMap(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cur,
  ...
  //   {
  //     std::cerr << "(" << cloud_cur->points.at(i).x << "," << cloud_cur->points.at(i).y << "," <<
  //     cloud_cur->points.at(i).z << ") ";
  //   }
...
int SurfaceHeightSaliencyMap::calculatePointDistanceMap(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cur,
                                                        pcl::PointIndices::Ptr indices_cur, int image_width,
                                                        int image_height, cv::Mat &map_cur) {
...
int SurfaceHeightSaliencyMap::combinePyramid(BasePyramid::Ptr pyramid) {
  for (unsigned int i = pyramid->getStartLevel(); i <= (unsigned int)pyramid->getMaxLevel(); ++i) {
    ...
    if ((heightType == AM_SHORT) || (heightType == AM_TALL)) {
      ...
      int rt_code =
          calculatePointDistanceMap(current_cloud, current_indices, current_width, current_height, current_map);
      if (rt_code != AM_OK) {
        return (rt_code);
      }
    } else {
