...
RelativeSurfaceOrientationMap::RelativeSurfaceOrientationMap() : BaseMap() {
  reset();
}
...
void RelativeSurfaceOrientationMap::reset() {
  ...
  orientation_normal.normal[0] = -1.0;
  orientation_normal.normal[1] = 0.0;
  orientation_normal.normal[2] = 0.0;
...
void RelativeSurfaceOrientationMap::print() {
  BaseMap::print();
  printf("[%s]: orientation_normal = (%f,%f,%f)\n", mapName.c_str(), orientation_normal.normal_x,
         orientation_normal.normal_y, orientation_normal.normal_z);
...
void RelativeSurfaceOrientationMap::setOrientationNormal(pcl::Normal orientation_normal_) {
  ...
  calculated = false;
  printf("[INFO]: %s: orientation_normal is set to: (%f,%f,%f)\n", mapName.c_str(), orientation_normal.normal[0],
         orientation_normal.normal[1], orientation_normal.normal[2]);
}
...
//   {
//     printf("[INFO]: %s: There should be 4 camera parameters! You have
...
//   calculated = false;
//   printf("[INFO]: %s: cameraParametrs is set to:
...
void RelativeSurfaceOrientationMap::orientationMap(pcl::PointCloud<pcl::Normal>::Ptr normals_cur,
                                                   pcl::PointIndices::Ptr indices_cur, int image_width,
                                                   int image_height, float a, float b, float c,
                                                   float orientationCoefficient, cv::Mat &map_cur) {
...
int RelativeSurfaceOrientationMap::combinePyramid(BasePyramid::Ptr pyramid) {
  ...
  for (unsigned int i = pyramid->getStartLevel(); i <= (unsigned int)pyramid->getMaxLevel(); ++i) {
    ...
    cv::Mat current_map;
    orientationMap(current_normals, current_indices, current_width, current_height, a, b, c, orientationCoefficient,
                   current_map);
