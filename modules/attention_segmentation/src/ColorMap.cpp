...
ColorSaliencyMap::ColorSaliencyMap() : BaseMap() {
  reset();
}
...
float ColorSaliencyMap::getMaxColorDistance(float &r_color, float &g_color, float &b_color, float &a_color) {
  ...
  if (useLAB) {
    ...
    // green color (0,1,0)
    float max2green =
        sqrt((r_color - 0) * (r_color - 0) + (g_color - 1) * (g_color - 1) + (b_color - 0) * (b_color - 0));
    // blue color (0,0,1)
    float max2blue =
        sqrt((r_color - 0) * (r_color - 0) + (g_color - 0) * (g_color - 0) + (b_color - 1) * (b_color - 1));
    // red-green color (1,1,0)
    float max2red_green =
        sqrt((r_color - 1) * (r_color - 1) + (g_color - 1) * (g_color - 1) + (b_color - 0) * (b_color - 0));
    // red-blue color (1,0,1)
    float max2red_blue =
        sqrt((r_color - 1) * (r_color - 1) + (g_color - 0) * (g_color - 0) + (b_color - 1) * (b_color - 1));
    // green-blue color (0,1,1)
    float max2green_blue =
        sqrt((r_color - 0) * (r_color - 0) + (g_color - 1) * (g_color - 1) + (b_color - 1) * (b_color - 1));
    // black color (0,0,0)
    float max2black =
        sqrt((r_color - 0) * (r_color - 0) + (g_color - 0) * (g_color - 0) + (b_color - 0) * (b_color - 0));
    // white color (1,1,1)
    float max2white =
        sqrt((r_color - 1) * (r_color - 1) + (g_color - 1) * (g_color - 1) + (b_color - 1) * (b_color - 1));
...
void ColorSaliencyMap::LabColorMap(cv::Mat &image_cur, int image_width, int image_height, float max_dist, float a_color,
...
void ColorSaliencyMap::RGBColorMap(cv::Mat &image_cur, int image_width, int image_height, float max_dist, float r_color,
                                   float g_color, float b_color, cv::Mat &map_cur) {
  map_cur = cv::Mat_<float>::zeros(image_height, image_width);
  ...
  for (int r = 0; r < image_height; ++r) {
    for (int c = 0; c < image_width; ++c) {
      // if(mask.at<uchar>(r,c))
      {
        ...
        float dist =
            (rr - r_color) * (rr - r_color) + (gg - g_color) * (gg - g_color) + (bb - b_color) * (bb - b_color);
        dist = sqrt(dist);
        map_cur.at<float>(r, c) = 1.0 - dist / max_dist;
      }
    }
