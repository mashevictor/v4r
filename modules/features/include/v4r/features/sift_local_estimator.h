 ...
 public:
  class Parameter {
   public:
    ...
    bool use_rootSIFT_;  ///< enables RootSIFT as described in Arandjelovic and Zisserman, Three things everyone should
    ...
    Parameter() : dense_extraction_(false), use_rootSIFT_(true), stride_(20) {}
  } param_;

#ifdef HAVE_SIFTGPU
  SIFTLocalEstimation(const boost::shared_ptr<SiftGPU> &sift)
  : max_distance_(std::numeric_limits<float>::max()), sift_(sift) {
    descr_name_ = "sift";
    descr_type_ = FeatureType::SIFT_GPU;
    descr_dims_ = 128;
  }

  SIFTLocalEstimation() : max_distance_(std::numeric_limits<float>::max()) {
    descr_name_ = "sift";
    descr_type_ = FeatureType::SIFT_GPU;
    descr_dims_ = 128;
  ...
  void compute(const cv::Mat_<cv::Vec3b> &colorImage, Eigen::Matrix2Xf &keypoints,
   ...
   */
  std::vector<std::pair<int, int>> matchSIFT(const std::vector<std::vector<float>> &desc1,
