...
class V4R_EXPORTS ImageKeypointExtractor {
 ...
 public:
  void setInputCloud(PointInTPtr &input) {
    input_ = input;
  }

  void setSupportRadius(float f) {
    radius_ = f;
  }

  virtual void compute(PointOutTPtr &keypoints) = 0;

  virtual void setNormals(const pcl::PointCloud<pcl::Normal>::Ptr & /*normals*/) {}

  virtual bool needNormals() {
    return false;
  }
};
...
class V4R_EXPORTS UniformSamplingExtractor : public KeypointExtractor<PointT> {
 private:
  ...
  void filterPlanar(const PointInTPtr &input, std::vector<int> &kp_idx) {
 ...
 public:
  void setFilterPlanar(bool b) {
    filter_planar_ = b;
  }

  void setSamplingDensity(float f) {
    sampling_density_ = f;
  }
...
class V4R_EXPORTS SIFTKeypointExtractor : public KeypointExtractor<PointInT> {
 ...
 public:
  void compute(typename pcl::PointCloud<PointT> &keypoints) {
...
class V4R_EXPORTS HarrisKeypointExtractor : public KeypointExtractor<PointInT> {
 ...
 public:
  ...
  void setResponseMethod(typename pcl::HarrisKeypoint3D<PointInT, pcl::PointXYZI>::ResponseMethod m) {
    m_ = m;
  }

  void setNonMaximaRadius(float r) {
    non_max_radius_ = r;
  }

  void compute(pcl::PointCloud<PointInT> &keypoints) {
    if (!normals_ || (normals_->points.size() != input_->points.size()))
...
class V4R_EXPORTS LocalEstimator {
 protected:
  ...
  void computeKeypoints(PointInTPtr &cloud, pcl::PointCloud<PointInT> &keypoints,
 ...
 public:
  ...
  virtual bool estimate(const PointInTPtr &in, PointInTPtr &processed, PointInTPtr &keypoints,
   ...
   */
  void addKeypointExtractor(boost::shared_ptr<KeypointExtractor<PointInT>> &ke) {
    keypoint_extractor_.push_back(ke);
  }

  void setKeypointExtractors(std::vector<typename boost::shared_ptr<KeypointExtractor<PointInT>>> &ke) {
    keypoint_extractor_ = ke;
  }

  void setSupportRadius(float r) {
    support_radius_ = r;
  }
  ...
  /*void
   ...
   if (tree->radiusSearch (keypoints_cloud[i], support_radius_, (*neighborhood_indices_)[good],
