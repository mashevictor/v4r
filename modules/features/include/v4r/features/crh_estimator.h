...
class V4R_EXPORTS CRHEstimation : public GlobalEstimator<PointInT, FeatureT> {
 ...
 public:
  ...
  void setFeatureEstimator(const typename boost::shared_ptr<GlobalEstimator<PointInT, FeatureT>>& feature_estimator) {
    feature_estimator_ = feature_estimator;
  }

  void estimate(const PointInTPtr& in, PointInTPtr& processed,
