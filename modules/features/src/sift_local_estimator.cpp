...
void SIFTLocalEstimation<PointT>::compute(std::vector<std::vector<float>> &signatures) {
...
template <typename PointT>
void SIFTLocalEstimation<PointT>::compute(const cv::Mat_<cv::Vec3b> &colorImage, Eigen::Matrix2Xf &keypoints,
...
#ifdef HAVE_SIFTGPU
template <typename PointT>
std::vector<std::pair<int, int>> SIFTLocalEstimation<PointT>::matchSIFT(const std::vector<std::vector<float>> &desc1,
