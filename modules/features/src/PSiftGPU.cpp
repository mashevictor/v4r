      ...
      if (mask.size() != image.size() || mask.type() != CV_8U) {
        ...
        for (int i = 0; i < num; i++)
          if (mask.at<uchar>((int)(ks[i].y + .5), (int)(ks[i].x + .5)) > 0) {
            keypoints.push_back(
                cv::KeyPoint(ks[i].x, ks[i].y, ks[i].s * 3, -ks[i].o * 180 / M_PI, 1, 0, keypoints.size()));
          }
      }
    } else
 ...
 */
void PSiftGPU::computeImpl(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) const {
  if (keypoints.size() == 0)
    return;
 ...
 */
void PSiftGPU::knnMatchImpl(const cv::Mat& queryDescriptors, vector<vector<cv::DMatch>>& matches, int k,
                            const vector<cv::Mat>& masks, bool compactResult) {
  (void)k;
  (void)masks;
  (void)compactResult;
  matches.clear();
  ...
  if (queryDescriptors.isContinuous())
    tmpQueryDesc = queryDescriptors;
  else
    queryDescriptors.copyTo(tmpQueryDesc);

  if (trainDescCollection.size() == 1) {
    if (trainDescCollection[0].isContinuous())
      tmpTrainDesc = trainDescCollection[0];
    else
      trainDescCollection[0].copyTo(tmpTrainDesc);
  } else {
    unsigned z = 0;
  ...
  num = matcher->GetSiftMatch(tmpQueryDesc.rows, match_buf, param.distmax, param.ratiomax, param.mutual_best_match);

  matches.resize(num, vector<cv::DMatch>(1));
  if (ltView.size() > 0) {
    for (unsigned i = 0; i < (unsigned)num; i++) {
      matches[i][0] =
          cv::DMatch(match_buf[i][0], ltKey[match_buf[i][1]], ltView[match_buf[i][1]],
                     Distance128(tmpQueryDesc.ptr<float>(match_buf[i][0]), tmpTrainDesc.ptr<float>(match_buf[i][1])));
    }
  } else {
    for (unsigned i = 0; i < (unsigned)num; i++) {
      matches[i][0] =
          cv::DMatch(match_buf[i][0], match_buf[i][1],
                     Distance128(tmpQueryDesc.ptr<float>(match_buf[i][0]), tmpTrainDesc.ptr<float>(match_buf[i][1])));
    }
  }
 ...
 * radiusMatchImpl
 */
void PSiftGPU::radiusMatchImpl(const cv::Mat& queryDescriptors, vector<vector<cv::DMatch>>& matches, float maxDistance,
                               const vector<cv::Mat>& masks, bool compactResult) {
  (void)masks;
  (void)compactResult;
  matches.clear();
  ...
  num = matcher->GetSiftMatch(tmpQueryDesc.rows, match_buf, maxDistance, param.ratiomax, param.mutual_best_match);
 ...
 * detect
 */
void PSiftGPU::detect(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, DataMatrix2Df& descriptors,
                      const cv::Mat& mask) {
  descriptors.clear();
  keypoints.clear();
  cv::Mat grayImage;
 ...
 * no mask used
 */
void PSiftGPU::match(const cv::Mat& queryDescriptors, const cv::Mat& trainDescriptors, vector<cv::DMatch>& matches,
                     const cv::Mat& mask) {
