...
bool best_clique_vectors(const std::pair<float, std::vector<size_t>*> a,
...
struct V4R_EXPORTS vertexDegreeSorter {
  bool operator()(const ViewD& i, const ViewD& j) const {
    return i < j;
  }
};
...
class V4R_EXPORTS save_cliques {
 public:
  /*save_cliques (std::size_t& max, std::size_t& maximum_clique, std::size_t& n_cliques, std::vector<std::vector<void *>
   *> & cliquess) :
  ...
  inline void clique(const Clique& c, Graph2& g) {
    if (c.size() >= min_size_) {
      BOOST_USING_STD_MAX();
...
bool best_extended_cliques(const ExtendedClique& a, const ExtendedClique& b) {
  /*float a_value = static_cast<float>(a.correspondences_->size()) * 0.5f + a.avg_descriptor_distance_ * 0.25f +
  a.avg_pair_3D_distance_ * 0.25f;
  float b_value = static_cast<float>(b.correspondences_->size()) * 0.5f + b.avg_descriptor_distance_ * 0.25f +
  b.avg_pair_3D_distance_ * 0.25f;*/
  ...
  /*float a_value = static_cast<float>(a.normalized_clique_size_) * 0.25f + a.avg_descriptor_distance_ * 0.25f +
  a.avg_pair_3D_distance_ * 0.25f + a.far_away_correspondences_weight_ * 0.25f;
  float b_value = static_cast<float>(b.normalized_clique_size_) * 0.25f + b.avg_descriptor_distance_ * 0.25f +
  b.avg_pair_3D_distance_ * 0.25f + b.far_away_correspondences_weight_ * 0.25f;*/

  float a_value = static_cast<float>(a.normalized_clique_size_) * 0.25f +
                  a.avg_descriptor_distance_ * 0.25f;  //+ a.far_away_correspondences_weight_ * 0.1f;
  float b_value = static_cast<float>(b.normalized_clique_size_) * 0.25f +
                  b.avg_descriptor_distance_ * 0.25f;  //+ b.far_away_correspondences_weight_ * 0.1f;
...
void GraphGeometricConsistencyGrouping<PointModelT, PointSceneT>::clusterCorrespondences(
  ...
  //    try
  {
    ...
    for (size_t c = 0; c < n_cc; c++) {
      ...
      // std::cout << "arboricity:" << arboricity << " num_v:" << num_v_in_cc << " edges:" << num_edges
      // (connected_graph) << std::endl;
      ...
      if (cliques_computation_possible[c] &&
        ...
        for (size_t k = 0; k < cliques.size(); k++) {
          extended_cliques[k].avg_pair_3D_distance_ =
              1.f - (extended_cliques[k].avg_pair_3D_distance_ / max_avg_3D_dist);
        ...
        // sort (cliques.begin (), cliques.end (), less_clique_vectors); //cliques are sorted in increasing order
        // (smaller cliques first)
              ...
              if (param_.check_normals_orientation_) {
                for (size_t j = 0; j < filtered_corrs.size(); j++) {
                  ...
                  if (pcl::isFinite(model_normal) && pcl::isFinite(scene_normal)) {
                    ...
                    if (model_normal_aligned.dot(scene_normal.getNormalVector3fMap()) >=
                        (1.f - param_.thres_dot_distance_))
                      all_wrong = false;
                  }
                }
              }
