...
bool EigenFLANN::nearestKSearch(const Eigen::MatrixXf &query_signature, Eigen::MatrixXi &indices,
  ...
  flann::Matrix<float> flann_distances(new float[query_signature.rows() * param_.knn_], query_signature.rows(),
                                       param_.knn_);

  indices.resize(query_signature.rows(), param_.knn_);
  distances.resize(query_signature.rows(), param_.knn_);

  flann::Matrix<float> query_desc(new float[query_signature.rows() * query_signature.cols()], query_signature.rows(),
                                  query_signature.cols());
  ...
  if (param_.distance_metric_ == 2) {
    if (!flann_index_l2_->knnSearch(query_desc, flann_indices, flann_distances, param_.knn_,
                                    flann::SearchParams(param_.kdtree_splits_)))
      return false;
  } else {
    if (!flann_index_l1_->knnSearch(query_desc, flann_indices, flann_distances, param_.knn_,
                                    flann::SearchParams(param_.kdtree_splits_)))
      return false;
  }
...
bool EigenFLANN::createFLANN(const Eigen::MatrixXf &data) {
