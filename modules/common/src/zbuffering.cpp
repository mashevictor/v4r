...
void ZBuffering<PointT>::renderPointCloud(const pcl::PointCloud<PointT> &cloud, pcl::PointCloud<PointT> &rendered_view,
  ...
  for (size_t i = 0; i < width * height; i++)  // initialize points to infinity
    rendered_view.points[i].x = rendered_view.points[i].y = rendered_view.points[i].z =
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < static_cast<int>(cloud.points.size()); i = i + subsample) {
    ...
    //        if ( smoothing_px_dist1st(v,u)<0.f || (dist_norm < smoothing_px_dist1st(v,u)) || (pt.z <
    //        (r_pt.z-param_.inlier_threshold_) ) )
