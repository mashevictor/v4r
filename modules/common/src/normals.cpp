...
V4R_EXPORTS void computeNormals(const typename pcl::PointCloud<PointT>::ConstPtr &cloud,
                                pcl::PointCloud<pcl::Normal>::Ptr &normals, int method, float radius) {
  CHECK(normals);
...
template V4R_EXPORTS void computeNormals<pcl::PointXYZRGB>(const pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr &,
                                                           pcl::PointCloud<pcl::Normal>::Ptr &, int, float);
template V4R_EXPORTS void computeNormals<pcl::PointXYZ>(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &,
                                                        pcl::PointCloud<pcl::Normal>::Ptr &, int, float);

#define PCL_INSTANTIATE_initNormalEstimator(T) \
  template V4R_EXPORTS typename NormalEstimator<T>::Ptr initNormalEstimator<T>(int, std::vector<std::string> &);
