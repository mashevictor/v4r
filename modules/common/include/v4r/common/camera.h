...
class V4R_EXPORTS Camera {
 protected:
  ...
  cv::Mat_<uchar> camera_depth_registration_mask_;  ///< this mask has the same size as the camera image and tells us
                                                    /// which pixels can have valid depth pixels and which ones are not
  /// seen due to the phsysical displacement between RGB and depth
  /// sensor.
  ...
  V4R_EXPORTS void serialize(Archive &ar, const unsigned int version) {
    (void)version;
    ar &BOOST_SERIALIZATION_NVP(width_) & BOOST_SERIALIZATION_NVP(height_) & BOOST_SERIALIZATION_NVP(fx_) &
        BOOST_SERIALIZATION_NVP(fy_) & BOOST_SERIALIZATION_NVP(cx_) & BOOST_SERIALIZATION_NVP(cy_) &
 ...
 public:
  ...
  Camera(float fx = 525.f, float fy = 525.f, size_t width = 640, size_t height = 480, float cx = 319.5f,
         float cy = 239.5f, float horizontal_fov_deg = 58.f)
  : width_(width), height_(height), fx_(fx), fy_(fy), cx_(cx), cy_(cy), horizontal_fov_deg_(horizontal_fov_deg) {
  ...
  /**
   * @brief setCameraDepthRegistrationMask this mask has the same size as the camera image and tells us which pixels can
   * have valid depth pixels and which ones are not seen due to the phsysical displacement between RGB and depth sensor.
  ...
  /**
   * @brief getCameraDepthRegistrationMask this mask has the same size as the camera image and tells us which pixels can
   * have valid depth pixels and which ones are not seen due to the phsysical displacement between RGB and depth sensor.
