...
#if !(defined WIN32 || defined _WIN32) && (defined __i486__ || defined __i586__ || defined __i686__ || \
...
class V4R_EXPORTS SmartPtr {
 public:
  ...
  SmartPtr(const SmartPtr<T2>& ptr);
  /* copy operator */
 ...
 */
template <typename T>
inline SmartPtr<T>::SmartPtr() : obj(0), refcount(0) {}

template <typename T>
inline SmartPtr<T>::SmartPtr(T* _obj) : obj(_obj) {
  if (obj) {
...
inline void SmartPtr<_Tp>::release() {
 ...
 */
template <typename T>
inline void SmartPtr<T>::delete_obj() {
  if (obj)
    delete obj;
}
 ...
 */
template <typename T>
inline SmartPtr<T>::SmartPtr(const SmartPtr<T>& _ptr) {
  obj = _ptr.obj;
 ...
 */
template <typename T>
inline SmartPtr<T>& SmartPtr<T>::operator=(const SmartPtr<T>& _ptr) {
...
template <typename T>
const inline T* SmartPtr<T>::get() const {
  return obj;
}

template <typename T>
inline void SmartPtr<T>::reset(T* _obj) {
  release();
...
template <typename T>
template <typename T2>
SmartPtr<T>::SmartPtr(const SmartPtr<T2>& p) : obj(0), refcount(0) {
  if (p.empty())
    return;
...
template <typename T>
template <typename T2>
inline SmartPtr<T2> SmartPtr<T>::ptr() {
  SmartPtr<T2> p;
  if (!obj)
    return p;
...
template <typename T>
template <typename T2>
inline const SmartPtr<T2> SmartPtr<T>::ptr() const {
  SmartPtr<T2> p;
  if (!obj)
    return p;
