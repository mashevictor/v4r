  ...
  public:
    ...
    Parameter()
     : win_size(cv::Size(21,21)), max_level(2), termcrit(cv::TermCriteria(CV_TERMCRIT_ITER|CV_TERMCRIT_EPS,20,0.03)), max_error(100),
       ...
       nnr(0.95), refine_plk(false), detect_loops(true), nb_tracked_frames(2), inl_dist(0.015),
...
private:
  ...
  int addProjectionsPLK(const std::vector<Eigen::Vector3f> &points3d, int frame_idx, const v4r::DataMatrix2D<Surfel> &sf_cloud, const Eigen::Matrix4f &inc_pose, const std::vector<cv::Point2f> &cv_points, const std::vector<int> &converged, std::vector< std::vector< v4r::triple<int, cv::Point2f, Eigen::Vector3f > > > &projs);
  void addLoops();
