...
public:
  ...
  public:
    ...
    bool tsf_mapping;             // collects keyframes and optionally optimizes the poses (post processing if optimizeMap is called)
...
public:
   ...
   */
  int getFilteredCloudNormals(pcl::PointCloud<pcl::PointXYZRGBNormal> &cloud, Eigen::Matrix4f &pose, uint64_t &timestamp);
   ...
   * @param _upscaling_ratio used to create upscaled RGBD frames in case Parameter::tsf_filtering or Parameter::tsf_mapping is activated
   ...
   * returns upscaled and optimized camera parameter (optimized in case optimizing intrinsics is activated and 'optimizeMap' is called before)
   * @param _intrinsic
   * @param _dist_coeffs
   */
