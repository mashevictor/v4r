...
public:
     ...
     * @param tex_mesh
     */
    void textureMapping(const std::vector< v4r::TSFFrame::Ptr > &frames, const pcl::PolygonMesh &mesh, const cv::Mat_<double> &intrinsic, const Eigen::Matrix4f &base_transform, const std::string &tmp_dir, const std::string &output_dir);

    /**
     * @brief setDeltaAngleViewNormal
     * @param delta_angle_view_normal
     */
    void setDeltaAngleViewNormal(const float &delta_angle_view_normal) { cosathr = cos(M_PI*delta_angle_view_normal/180.); }
...
private:
     ...
     */
    void smoothNormalsClusterFaces(const pcl::PointCloud<pcl::PointXYZ> &cloud, const std::vector<pcl::Vertices> &faces, std::vector<Eigen::Vector3f> &face_normals);
    int getNbFace(const std::vector< std::vector<int> > &face_indices, const std::vector<pcl::Vertices> &faces, int idx_face, int idx_vertex0, int idx_vertex1);
    void clusterFaces(const std::vector< std::vector<int> > &face_nbs, int idx, const std::vector<Eigen::Vector3f> &face_normals, std::vector<int> &mask, Eigen::Vector3f &normal, int &cnt, std::vector<int> &indices);
    void smoothNormalsTriangleNeighbours(const std::vector< std::vector<int> > &face_nbs, std::vector<Eigen::Vector3f> &face_normals);
...
inline void OdmTexturing::addOrientedMeanNormal(const Eigen::Vector3f &normal, Eigen::Vector3f &mean_normal, int &cnt)
{
  ...
  else mean_normal = ((float)cnt)*mean_normal + normal;
