...
OdmTexturing::OdmTexturing(const float &delta_angle_view_normal) : log_(false) {
...
/**
 * @brief OdmTexturing::textureMapping
 * @param frames
 * @param mesh
 * @param dir
 * @param tex_mesh
 */
void OdmTexturing::textureMapping(const std::vector<v4r::TSFFrame::Ptr> &frames, const pcl::PolygonMesh &mesh,
                                  const cv::Mat_<double> &intrinsic, const Eigen::Matrix4f &base_transform,
                                  const std::string &tmp_dir, const std::string &output_dir) {
  // setup tex mesh model
  outputFolder_ = output_dir;
  boost::filesystem::create_directories(outputFolder_);

  mesh_->cloud = mesh.cloud;
  ...
  std::string image_names = tmp_dir + "/tex_%08d.png";
...
void OdmTexturing::parseArguments(int argc, char **argv) {
  ...
  if (textureWithSize_ > textureResolution_) {
    ...
    log_ << "textureWithSize parameter was set to a lower value since it can not be greater than the texture "
            "resolution.\n";
  }
}

void OdmTexturing::loadMesh() {
  cout << "[OdmTexturing::loadMesh] TODO: loadPLYFile is not available in the stable version of pcl?" << endl;
...
void OdmTexturing::loadCameras() {
  ...
  for (int i = 0; i < nrCameras; ++i) {
    ...
    bundleFile >> val;
    transform(0, 0) = val;  // Read rotation (0,0) from bundle file
    bundleFile >> val;
    transform(0, 1) = val;  // Read rotation (0,1) from bundle file
    bundleFile >> val;
    transform(0, 2) = val;  // Read rotation (0,2) from bundle file

    bundleFile >> val;
    transform(1, 0) = val;  // Read rotation (1,0) from bundle file
    bundleFile >> val;
    transform(1, 1) = val;  // Read rotation (1,1) from bundle file
    bundleFile >> val;
    transform(1, 2) = val;  // Read rotation (1,2) from bundle file

    bundleFile >> val;
    transform(2, 0) = val;  // Read rotation (2,0) from bundle file
    bundleFile >> val;
    transform(2, 1) = val;  // Read rotation (2,1) from bundle file
    bundleFile >> val;
    transform(2, 2) = val;  // Read rotation (2,2) from bundle file

    bundleFile >> val;
    transform(0, 3) = val;  // Read translation (0,3) from bundle file
    bundleFile >> val;
    transform(1, 3) = val;  // Read translation (1,3) from bundle file
    bundleFile >> val;
    transform(2, 3) = val;  // Read translation (2,3) from bundle file
...
int OdmTexturing::getNbFace(const std::vector<std::vector<int>> &face_indices, const std::vector<pcl::Vertices> &faces,
                            int idx_face, int idx_vertex0, int idx_vertex1) {
  for (unsigned i = 0; i < face_indices[idx_vertex0].size(); i++) {
    if (face_indices[idx_vertex0][i] == idx_face)
      continue;

    const pcl::Vertices &vs = faces[face_indices[idx_vertex0][i]];

    if (((int)vs.vertices[0]) == idx_vertex1 || ((int)vs.vertices[1]) == idx_vertex1 ||
        ((int)vs.vertices[2]) == idx_vertex1)
      return face_indices[idx_vertex0][i];
  }
  return -1;
}

void OdmTexturing::clusterFaces(const std::vector<std::vector<int>> &face_nbs, int idx,
                                const std::vector<Eigen::Vector3f> &face_normals, std::vector<int> &mask,
                                Eigen::Vector3f &normal, int &cnt, std::vector<int> &indices) {
  indices.push_back(idx);
  mask[idx] = mcnt;
  addOrientedMeanNormal(face_normals[idx], normal, cnt);

  for (unsigned i = 0; i < face_nbs[idx].size(); i++) {
    if (mask[face_nbs[idx][i]] != mcnt && fabs(normal.dot(face_normals[face_nbs[idx][i]])) > cosathr)
      clusterFaces(face_nbs, face_nbs[idx][i], face_normals, mask, normal, cnt, indices);
  }
}

void OdmTexturing::smoothNormalsTriangleNeighbours(const std::vector<std::vector<int>> &face_nbs,
                                                   std::vector<Eigen::Vector3f> &face_normals) {
  std::vector<Eigen::Vector3f> tmp_normals = face_normals;

  for (unsigned i = 0; i < tmp_normals.size(); i++) {
    int cnt = 1;
    for (unsigned j = 0; j < face_nbs[i].size(); j++)
      addOrientedMeanNormal(face_normals[face_nbs[i][j]], face_normals[i], cnt);
  }
}

void OdmTexturing::smoothNormalsClusterFaces(const pcl::PointCloud<pcl::PointXYZ> &cloud,
                                             const std::vector<pcl::Vertices> &faces,
                                             std::vector<Eigen::Vector3f> &face_normals) {
  // create graph to neighbouring faces
  int nb_face;
  std::vector<std::vector<int>> face_indices(cloud.points.size());
  std::vector<std::vector<int>> face_nbs(faces.size());

  for (unsigned i = 0; i < faces.size(); i++) {
    face_indices[faces[i].vertices[0]].push_back(i);
    face_indices[faces[i].vertices[1]].push_back(i);
    face_indices[faces[i].vertices[2]].push_back(i);
  }

  for (unsigned i = 0; i < faces.size(); i++) {
    nb_face = getNbFace(face_indices, faces, i, faces[i].vertices[0], faces[i].vertices[1]);
    if (nb_face != -1)
      face_nbs[i].push_back(nb_face);
    nb_face = getNbFace(face_indices, faces, i, faces[i].vertices[1], faces[i].vertices[2]);
    if (nb_face != -1)
      face_nbs[i].push_back(nb_face);
    nb_face = getNbFace(face_indices, faces, i, faces[i].vertices[2], faces[i].vertices[0]);
    if (nb_face != -1)
      face_nbs[i].push_back(nb_face);
  }

  smoothNormalsTriangleNeighbours(face_nbs, face_normals);

  // do recursive clustering
  mcnt = 1;
  std::vector<int> mask(faces.size(), 0);
  std::vector<int> indices;

  for (unsigned i = 0; i < face_nbs.size(); i++) {
    if (mask[i] != mcnt) {
      int cnt = 0;
      indices.clear();

      clusterFaces(face_nbs, i, face_normals, mask, face_normals[i], cnt, indices);

      for (unsigned j = 0; j < indices.size(); j++)
        face_normals[indices[j]] = face_normals[i];
    }
  }
}
...
void OdmTexturing::triangleToImageAssignment() {
  ...
  const pcl::PointCloud<pcl::PointXYZ> &pts = *meshCloud;
  const std::vector<pcl::Vertices> &faces = mesh_->tex_polygons[0];
  std::vector<Eigen::Vector3f> face_normals(faces.size());
  std::vector<Eigen::Vector3f> cam_face_normals(faces.size());
  std::vector<float> delta_cosa_view(faces.size(), 0);

  for (unsigned i = 0; i < faces.size(); i++) {
    l1 = pts.points[faces[i].vertices[1]].getVector3fMap() - pts.points[faces[i].vertices[0]].getVector3fMap();
    l2 = pts.points[faces[i].vertices[2]].getVector3fMap() - pts.points[faces[i].vertices[1]].getVector3fMap();
    face_normals[i] = l1.cross(l2).normalized();
  }

  smoothNormalsClusterFaces(pts, faces, face_normals);
  ...
  for (size_t cameraIndex = 0; cameraIndex < cameras_.size(); ++cameraIndex) {
    ...
    // rotate face normals to camera
    Eigen::Matrix3f R = cameras_[cameraIndex].pose.inverse().matrix().topLeftCorner<3, 3>();
    for (unsigned i = 0; i < face_normals.size(); i++)
      cam_face_normals[i] = R * face_normals[i];
    ...
    for (size_t faceIndex = 0; faceIndex < mesh_->tex_polygons[0].size(); ++faceIndex) {
      ...
      pcl::PointXY pixelPos0;
      pcl::PointXY pixelPos1;
      pcl::PointXY pixelPos2;

      // If the face is inside the camera frustum
      if (isFaceProjected(cameras_[cameraIndex], cameraCloud->points[mesh_->tex_polygons[0][faceIndex].vertices[0]],
                          cameraCloud->points[mesh_->tex_polygons[0][faceIndex].vertices[1]],
                          cameraCloud->points[mesh_->tex_polygons[0][faceIndex].vertices[2]], pixelPos0, pixelPos1,
                          pixelPos2)) {
    ...
    for (size_t faceIndex = 0; faceIndex < visibility.size(); ++faceIndex) {
      if (visibility[faceIndex]) {
        float delta_angle =
            getAbsCosaDeltaAngle(cameraCloud->points[faces[faceIndex].vertices[0]].getVector3fMap().normalized(),
                                 cam_face_normals[faceIndex]);

        if (delta_angle > delta_cosa_view[faceIndex]) {
          delta_cosa_view[faceIndex] = delta_angle;
...
void OdmTexturing::calculatePatches() {
  ...
  // Setup vector containing vectors with all faces correspondning to camera according to triangleToImageAssignment
  // vector
  for (size_t i = 0; i < tTIA_.size(); ++i) {
    if (tTIA_[i] > -1) {
  ...
  for (size_t cameraIndex = 0; cameraIndex < cameras_.size(); ++cameraIndex) {
    ...
    while (0 < optFaceCameraVector[cameraIndex].size()) {
      ...
      pcl::PointXY uvCoord1;
      pcl::PointXY uvCoord2;
      pcl::PointXY uvCoord3;
      ...
      if (isFaceProjected(cameras_[cameraIndex],
                          ...
                          cameraCloud->points[mesh_->tex_polygons[0][addedFaces.back()].vertices[2]], uvCoord1,
                          uvCoord2, uvCoord3)) {
            ...
            if (modelFaceIndex != patchFaceIndex) {
              ...
              if (nShared > 0) {
                ...
                isFaceProjected(cameras_[cameraIndex],
                                ...
                                cameraCloud->points[mesh_->tex_polygons[0][modelFaceIndex].vertices[2]], uv1, uv2, uv3);
  ...
  for (int textureIndex = 0; textureIndex < nrTextures_; ++textureIndex) {
    for (size_t patchIndex = 0; patchIndex < patches_.size(); ++patchIndex) {
      ...
      if (patches_[patchIndex].materialIndex_ == textureIndex) {
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr cameraCloud(new pcl::PointCloud<pcl::PointXYZ>);
        pcl::transformPointCloud(*meshCloud, *cameraCloud,
                                 cameras_[patches_[patchIndex].optimalCameraIndex_].pose.inverse());
        ...
        for (size_t faceIndex = 0; faceIndex < patches_[patchIndex].faces_.size(); ++faceIndex) {
          ...
          getPixelCoordinates(cameraCloud->points[mesh_->tex_polygons[0][globalFaceIndex].vertices[0]],
                              cameras_[patches_[patchIndex].optimalCameraIndex_], pixelPos0);
          getPixelCoordinates(cameraCloud->points[mesh_->tex_polygons[0][globalFaceIndex].vertices[1]],
                              cameras_[patches_[patchIndex].optimalCameraIndex_], pixelPos1);
          getPixelCoordinates(cameraCloud->points[mesh_->tex_polygons[0][globalFaceIndex].vertices[2]],
                              cameras_[patches_[patchIndex].optimalCameraIndex_], pixelPos2);
    ...
    meshMaterial.tex_Kd.r = 0.0f;
    meshMaterial.tex_Kd.g = 0.0f;
    meshMaterial.tex_Kd.b = 0.0f;
    meshMaterial.tex_Ks.r = 0.0f;
    meshMaterial.tex_Ks.g = 0.0f;
    meshMaterial.tex_Ks.b = 0.0f;
  ...
  for (int textureIndex = 0; textureIndex < nrTextures_; ++textureIndex) {
    ...
    for (int cameraIndex = 0; cameraIndex < static_cast<int>(cameras_.size()); ++cameraIndex) {
      ...
      for (size_t patchIndex = 0; patchIndex < patches_.size(); ++patchIndex) {
        ...
        if (patches_[patchIndex].materialIndex_ == textureIndex &&
            patches_[patchIndex].optimalCameraIndex_ == cameraIndex) {
...
void OdmTexturing::writeObjFile() {
  if (saveOBJFile(outputFolder_ + "tex_mesh.obj", *mesh_.get(), 7) == 0) {
    log_ << "tex_mesh.obj successfully saved.\n";
  } else {
...
void OdmTexturing::printHelp() {
  ...
  log_ << "Usage:\n";
  log_ << "The program requires paths to a bundler.out file, a ply model file, an image list file, an image folder "
          "path, an output path and the resized resolution used in the bundler step.\n";
  log_ << "All other input parameters are optional.\n\n";

  log_ << "The following flags are available:\n";
  log_ << "Call the program with flag \"-help\", or without parameters to print this message, or check any generated "
          "log file.\n";
  log_ << "Call the program with flag \"-verbose\", to print log messages in the standard output.\n\n";

  log_ << "Parameters are specified as: \"-<argument name> <argument>\", (without <>), and the following parameters "
          "are configurable:\n";
