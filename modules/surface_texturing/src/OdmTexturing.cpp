...
OdmTexturing::OdmTexturing(const float &delta_angle_view_normal)
  : log_(false)
{
 ...
 * @param tex_mesh
 */
void OdmTexturing::textureMapping(const std::vector< v4r::TSFFrame::Ptr > &frames, const pcl::PolygonMesh &mesh, const cv::Mat_<double> &intrinsic, const Eigen::Matrix4f &base_transform, const std::string &tmp_dir, const std::string &output_dir)
{
...
int OdmTexturing::getNbFace(const std::vector< std::vector<int> > &face_indices, const std::vector<pcl::Vertices> &faces, int idx_face, int idx_vertex0, int idx_vertex1)
{
  for (unsigned i=0; i<face_indices[idx_vertex0].size(); i++)
  {
    ...
    if (((int)vs.vertices[0])==idx_vertex1 || ((int)vs.vertices[1])==idx_vertex1 || ((int)vs.vertices[2])==idx_vertex1)
      return face_indices[idx_vertex0][i];
  }
  return -1;
}

void OdmTexturing::clusterFaces(const std::vector< std::vector<int> > &face_nbs, int idx, const std::vector<Eigen::Vector3f> &face_normals, std::vector<int> &mask, Eigen::Vector3f &normal, int &cnt, std::vector<int> &indices)
{
...
void OdmTexturing::smoothNormalsTriangleNeighbours(const std::vector< std::vector<int> > &face_nbs, std::vector<Eigen::Vector3f> &face_normals)
{
...
void OdmTexturing::smoothNormalsClusterFaces(const pcl::PointCloud<pcl::PointXYZ> &cloud, const std::vector<pcl::Vertices> &faces, std::vector<Eigen::Vector3f> &face_normals)
{
  ...
  for (unsigned i=0; i<faces.size(); i++)
  {
    ...
    if (nb_face!=-1) face_nbs[i].push_back(nb_face);
...
void OdmTexturing::triangleToImageAssignment()
{
    ...
    for (size_t cameraIndex = 0; cameraIndex < cameras_.size(); ++cameraIndex)
    {
        ...
        for (size_t faceIndex = 0; faceIndex < visibility.size();++faceIndex)
        {
            if (visibility[faceIndex])
            {
              float delta_angle = getAbsCosaDeltaAngle(cameraCloud->points[faces[faceIndex].vertices[0]].getVector3fMap().normalized(), cam_face_normals[faceIndex]);
