...
inline bool FastIterativeClosestPointWithGC<PointT>::filterHypothesesByPose(
    typename boost::shared_ptr<ICPNode<PointT>>& current,
...
inline void FastIterativeClosestPointWithGC<PointT>::visualizeICPNodes(
    typename std::vector<boost::shared_ptr<ICPNode<PointT>>>& nodes, pcl::visualization::PCLVisualizer& icp_vis) {
...
std::vector<float> FastIterativeClosestPointWithGC<PointT>::evaluateHypotheses(
    PointTPtr& im1, PointTPtr& im_2, pcl::PointCloud<pcl::Normal>::Ptr& normals1,
    pcl::PointCloud<pcl::Normal>::Ptr& normals_2, Eigen::Matrix4f pose_2_to_1) {
...
template <typename PointT>
void FastIterativeClosestPointWithGC<PointT>::align(Eigen::Matrix4f initial_guess) {
  ...
  typename std::vector<boost::shared_ptr<ICPNode<PointT>>>
  ...
  do {
    ...
    // now its time to evaluate the hypotheses!
    {
      ...
      for (size_t k = 0; k < next_level_nodes_.size(); k++) {
        int max_points = std::min(static_cast<int>(next_level_nodes_[k]->src_keypoints_->points.size()),
                                  static_cast<int>(tgt_keypoints->points.size()));
        ...
        float fsv_ij =
            vr.computeFSV(target_, next_level_nodes_[k]->src_keypoints_);  //, next_level_nodes_[k]->accum_transform_);
        ...
        if (next_level_nodes_[k]->overlap_ > 0) {
          ...
          float ov =
              std::min(static_cast<float>(next_level_nodes_[k]->overlap_), max_ov) / static_cast<float>(max_points);
