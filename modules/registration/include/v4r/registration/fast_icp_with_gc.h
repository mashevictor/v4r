...
class V4R_EXPORTS ICPNode {
 ...
 public:
  ...
  void addChild(typename boost::shared_ptr<ICPNode<PointT>>& c) {
    childs_.push_back(c);
  }
...
class V4R_EXPORTS FastIterativeClosestPointWithGC {
  ...
  void visualizeICPNodes(typename std::vector<boost::shared_ptr<ICPNode<PointT>>>& nodes,
                         pcl::visualization::PCLVisualizer& vis);

  bool filterHypothesesByPose(typename boost::shared_ptr<ICPNode<PointT>>& current,
                              typename std::vector<boost::shared_ptr<ICPNode<PointT>>>& nodes, float trans_threshold);

  std::vector<float> evaluateHypotheses(PointTPtr& im1, PointTPtr& im_2, pcl::PointCloud<pcl::Normal>::Ptr& normals1,
  ...
  inline void transformNormals(pcl::PointCloud<pcl::Normal>::Ptr& normals_cloud, Eigen::Matrix4f& transform) {
  ...
  inline void transformNormals(pcl::PointCloud<pcl::Normal>::Ptr& normals_cloud,
                               pcl::PointCloud<pcl::Normal>::Ptr& normals_out, Eigen::Matrix4f& transform) {
    normals_out->points.resize(normals_cloud->points.size());
    for (size_t k = 0; k < normals_cloud->points.size(); k++) {
      Eigen::Vector3f nt(normals_cloud->points[k].normal_x, normals_cloud->points[k].normal_y,
                         normals_cloud->points[k].normal_z);
      normals_out->points[k].normal_x =
          static_cast<float>(transform(0, 0) * nt[0] + transform(0, 1) * nt[1] + transform(0, 2) * nt[2]);
      normals_out->points[k].normal_y =
          static_cast<float>(transform(1, 0) * nt[0] + transform(1, 1) * nt[1] + transform(1, 2) * nt[2]);
      normals_out->points[k].normal_z =
          static_cast<float>(transform(2, 0) * nt[0] + transform(2, 1) * nt[1] + transform(2, 2) * nt[2]);
    }
  }

  inline void uniformSamplingOfKeypoints(typename pcl::PointCloud<PointT>::Ptr& keypoint_cloud,
                                         std::vector<int>& indices_keypoints, std::vector<int>& indices,
                                         v4r::UniformSamplingSharedVoxelGrid<PointT>& keypoint_extractor) {
  ...
  inline void computeRGBEdges(typename pcl::PointCloud<PointT>::Ptr& cloud, std::vector<int>& indices) {
  ...
  inline void getMask(typename pcl::PointCloud<PointT>::Ptr& cloud, std::vector<int>& indices,
  ...
  inline void getKeypointsWithMask(typename pcl::PointCloud<PointT>::Ptr& cloud, std::vector<int>& indices,
                                   std::vector<int>& roi_indices, std::vector<int>& indices_out) {
    std::vector<bool> mask;
