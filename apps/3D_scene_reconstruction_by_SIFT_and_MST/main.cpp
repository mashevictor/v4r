...
class scene_registration {
 ...
 public:
  ...
  void calcSiftFeatures(const typename pcl::PointCloud<PointT>::Ptr &cloud_src, std::vector<int> &sift_keypoint_indices,
      ...
      for (size_t view_b = 0; view_b < view_a; view_b++) {
        ...
        std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> sift_transforms =
            v4r::Registration::FeatureBasedRegistration<PointT>::estimateViewTransformationBySIFT(
                *grph_[view_a].cloud_, *grph_[view_b].cloud_, grph_[view_a].sift_keypoint_indices_,
                grph_[view_b].sift_keypoint_indices_, grph_[view_a].sift_signatures_, grph_[view_b].sift_signatures_);
        ...
        for (size_t trans_id = 0; trans_id < transforms.size(); trans_id++) {
          ...
          calcEdgeWeightAndRefineTf(grph_[view_a].cloud_, grph_[view_b].cloud_, transforms[trans_id].transformation_,
                                    transforms[trans_id].edge_weight, icp_refined_trans);
        ...
        boost::add_edge(transforms[best_transform_id].source_id_, transforms[best_transform_id].target_id_,
                        transforms[best_transform_id], gs_);
      }
    }
  }
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> getAbsolutePoses() {
...
int main(int argc, char **argv) {
  ...
  for (size_t sub_folder_id = 0; sub_folder_id < sub_folder_names.size(); sub_folder_id++) {
    ...
    if (files_intern.empty()) {
      std::cerr << "No files in directory: " << path_sub << ". Usage " << argv[0]
                << " -path <folder name> -rows <number of rows used for displaying files>" << std::endl;
      return -1;
    }
