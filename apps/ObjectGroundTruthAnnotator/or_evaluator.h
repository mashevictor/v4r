...
class OREvaluator {
 private:
  ...
  typename std::map<SceneId, boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>>
      transforms_;

  /*typename std::map<SceneId, boost::shared_ptr<std::vector<ModelTPtr> > > recognition_results_upper_bound_;
  typename std::map<SceneId, boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> >
  > > transforms_upper_bound_;*/
 ...
 public:
  ...
  void setDataSource(typename boost::shared_ptr<v4r::Source<ModelPointT>> source) {
    source_ = source;
  }
  ...
  void loadGTData();
  void addRecognitionResults(std::vector<std::string>& model_ids, std::vector<Eigen::Matrix4f>& poses);

  void addRecognitionResults(
  ...
  void classify(SceneId& id, boost::shared_ptr<std::vector<ModelTPtr>>& results,
                boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>& transforms,
                std::vector<bool>& correct);

  void getGroundTruthModelsAndPoses(
      SceneId& id, boost::shared_ptr<std::vector<ModelTPtr>>& results,
      boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>& transforms);

  void computeStatistics();

  void computeStatisticsUpperBound();

  void visualizeGroundTruth(pcl::visualization::PCLVisualizer& vis, SceneId& s_id, int viewport = -1, bool clear = true,
                            std::string cloud_name = "gt_model_cloud");

  void getGroundTruthPointCloud(SceneId& s_id, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& gt_cloud,
                                float model_res = -1.f);

  void getModelsForScene(SceneId& s_id, std::vector<typename pcl::PointCloud<ModelPointT>::Ptr>& model_clouds,
                         float res = 0.003f);
  ...
  void setGTDir(std::string& dir) {
    gt_dir_ = dir;
  }

  void setScenesDir(std::string& dir) {
    scenes_dir_ = dir;
  }

  void setModelFileExtension(std::string aa) {
    model_file_extension_ = aa;
  }

  void setReplaceModelExtension(bool b) {
