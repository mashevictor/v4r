...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::computeOcclusionValues(
    bool only_if_not_exist, bool do_icp_) {
  ...
  for (gt_data_main_iterator = gt_data_.begin(); gt_data_main_iterator != gt_data_.end(); gt_data_main_iterator++) {
    ...
    while (scene_map_iterator != gt_data_main_iterator->second.end()) {
      ...
      for (size_t i = 0; i < scene_map_iterator->second.size(); i++) {
        ...
        occ_file << gt_dir_ << "/" << gt_data_main_iterator->first << "_occlusion_" << model_name << "_"
                 << scene_map_iterator->second[i]->inst_ << ".txt";
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::refinePoses() {
  std::cout << "computeOcclusionValues" << std::endl;
  if (!checkLoaded())
    return;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::loadGTData() {
    ...
    for (size_t m = 0; m < model_files.size(); m++) {
      ...
      for (size_t inst = 0; inst < max_inst; inst++) {
        ...
        if (bf::exists(pose_path)) {
          ...
          std::stringstream occ_file;
          occ_file << gt_dir_ << "/" << scene_path << "_occlusion_" << model_files_wo_extension[m] << "_" << inst
                   << ".txt";
...
//            pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> model_handler (model_aligned, 125, 125,
125);
//            vis.addPointCloud(model_aligned, model_handler, "lolo");
//            vis.addPointCloud(scene, "scene");
//            {
...
//            vis.spin();
          ...
          if (gt_data_main_iterator == gt_data_.end()) {
            ...
            if (scene_map_iterator == gt_data_main_iterator->second.end()) {
              std::vector<GTModelTPtr> vec;
              vec.push_back(model_inst);
              gt_data_main_iterator->second.insert(
                  std::pair<std::string, std::vector<GTModelTPtr>>(model_files[m], vec));
            } else {
...
template <typename ModelPointT, typename SceneId>
int faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT,
                                                          SceneId>::countTotalNumberOfObjectsSequenceWise() {
  // get scenes in scene_dir
  std::vector<std::string> scene_files;
  std::vector<std::string> model_files;
  std::vector<std::string> model_files_wo_extension;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::addRecognitionResults(
    std::vector<std::string>& model_ids, std::vector<Eigen::Matrix4f>& poses) {}

template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::saveRecognitionResults(
    std::string& out_dir) {
  ...
  typename std::map<
      SceneId, boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>>::iterator
      results_transforms_it;

  results_transforms_it = transforms_.begin();

  for (results_it = recognition_results_.begin(); results_it != recognition_results_.end();
       results_it++, results_transforms_it++) {
    typename std::pair<SceneId, boost::shared_ptr<std::vector<ModelTPtr>>> rr = *results_it;
    typename std::pair<SceneId,
                       boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>>
        rtrans = *results_transforms_it;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::classify(
    SceneId& id, boost::shared_ptr<std::vector<ModelTPtr>>& results,
  ...
  transforms_.insert(
      std::pair<SceneId, boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>>(
          id, transforms));
  ...
  for (results_it = recognition_results_.begin(); results_it != recognition_results_.end();
    ...
    typename std::map<std::string, std::vector<GTModelTPtr>>::iterator
        gt_scene_map_iterator;  // first is model, second are model instances
          ...
          if (model_id.compare((*recog_id)->id_) == 0) {
            if (!check_pose_) {
              ...
              if (diff > max_centroid_diff_) {
                std::cout << "Id ok but rejected by centroid distances: " << model_id << " " << diff << " "
              ...
              if (check_rotation_) {
                ...
                if (max_angle > max_rotation_) {
                  std::cout << "Id and centroid ok but rejected by rotation: " << model_id << " " << max_angle << " "
                            << max_rotation_ << std::endl;
                  continue;
                }
              }
...
template <typename ModelPointT, typename SceneId>
bool faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::checkLoaded() {
  return gt_data_loaded_;
}

template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::computeStatistics() {
  if (!checkLoaded())
    return;
          ...
          if (model_id.compare((*recog_id)->id_) == 0) {
            if (!check_pose_) {
              ...
              if (check_rotation_) {
                ...
                std::cout << "angles:" << angle_x << " " << angle_y << " " << angle_z << "avg:" << avg_angle
                          << std::endl;
                std::cout << "max rotation:" << max_rotation_ << std::endl;

                if (avg_angle > max_rotation_) {
                  std::cout << "Id and centroid ok but rejected by rotation: " << model_id << " " << avg_angle << " "
                            << max_rotation_ << std::endl;
                  continue;
                }
              }
  ...
  std::cout << "Average centroid diff:" << avg_diff / static_cast<float>(instances) << " min,max:" << min_diff << ","
            << max_diff << "   " << min_diff_scene_id << "," << max_diff_scene_id << std::endl;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::computeStatisticsUpperBound() {
  if (!checkLoaded())
    return;
      ...
      for (size_t i = 0; i < gt_scene_map_iterator->second.size(); i++)  // model instances for GT data, select best!
      {
        ...
        for (recog_id = rr.second->begin(); (recog_id != rr.second->end()); recog_id++, idx++) {
          ...
          if (model_id.compare((*recog_id)->id_) == 0) {
            ...
            float pose_factor = (0.75f * ((max_centroid_diff_ - diff) / max_centroid_diff_) +
                                 0.25f * (std::max(0.f, 180.f - angle)) / 180.f);
        ...
        if (model_found) {
          ...
          // recognition_results_upper_bound_.insert(std::make_pair(gt_scene_map_iterator->first,
          // rr.second->at(idx_found));
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::saveStatistics(
    std::string& out_file) {
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::savePoseStatistics(
    std::string& out_file) {
  typename std::map<SceneId, PoseStatistics>::iterator pose_iterator;
  float avg_diff = 0.f;
  float max_diff = -1.f;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::savePoseStatisticsRotation(
    std::string& out_file) {
  typename std::map<SceneId, PoseStatistics>::iterator pose_iterator;
  float avg_diff = 0.f;
  float max_diff = -1.f;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::updateGT(std::string& sid,
                                                                                           std::string& m_id,
                                                                                           Eigen::Matrix4f& pose) {
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::getModelsForScene(
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::getGroundTruthModelsAndPoses(
    SceneId& s_id, boost::shared_ptr<std::vector<ModelTPtr>>& results,
    boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>>& transforms) {
  if (!checkLoaded())
    return;
  ...
  gt_data_main_iterator = gt_data_.find(s_id);
  if (gt_data_main_iterator == gt_data_.end()) {
    std::cout << "Scene id not found... in getGroundTruthModelsAndPoses:" << s_id << " " << gt_data_.size()
              << std::endl;
    for (gt_data_main_iterator = gt_data_.begin(); gt_data_main_iterator != gt_data_.end(); gt_data_main_iterator++) {
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::getGroundTruthPointCloud(
    SceneId& s_id, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& gt_cloud, float model_res) {
  if (!checkLoaded())
    return;
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::visualizeGroundTruth(
    ...
    while (scene_map_iterator != gt_data_main_iterator->second.end()) {
      for (size_t i = 0; i < scene_map_iterator->second.size(); i++) {
        ...
        pcl::for_each_type<FieldListM>(pcl::CopyIfFieldExists<typename CloudM::PointType, float>(
            model_aligned->points[0], "rgb", exists_m, rgb_m));
...
template <typename ModelPointT, typename SceneId>
void faat_pcl::rec_3d_framework::or_evaluator::OREvaluator<ModelPointT, SceneId>::copyToDirectory(
    std::string& out_dir) {
  typename std::map<SceneId, std::map<std::string, std::vector<GTModelTPtr>>>::iterator gt_data_main_iterator;
  for (gt_data_main_iterator = gt_data_.begin(); gt_data_main_iterator != gt_data_.end(); gt_data_main_iterator++) {
    ...
    while (scene_map_iterator != gt_data_main_iterator->second.end()) {
      std::cout << "scene_map_iterator->first:" << scene_map_iterator->first << " " << scene_map_iterator->second.size()
                << std::endl;  // model id
