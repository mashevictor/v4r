...
void mouse_callback_scenes(const pcl::visualization::MouseEvent &mouse_event, void *cookie) {
  MainWindow *main_w = (MainWindow *)cookie;

  if (mouse_event.getType() == pcl::visualization::MouseEvent::MouseDblClick &&
    ...
    cout << "Clicked in scene window :: " << mouse_event.getX() << " , " << mouse_event.getY() << " , " << scene_clicked
         << endl;
...
void mouse_callback_models(const pcl::visualization::MouseEvent &mouse_event, void *cookie) {
  MainWindow *main_w = (MainWindow *)cookie;

  if (mouse_event.getType() == pcl::visualization::MouseEvent::MouseDblClick &&
      mouse_event.getButton() == pcl::visualization::MouseEvent::LeftButton) {
    int model_clicked = round(mouse_event.getX() / main_w->getModelXSize());
    cout << "Clicked in model window :: " << mouse_event.getX() << " , " << mouse_event.getY() << " , " << model_clicked
         << endl;
...
void MainWindow::lock_with_icp() {
  ...
  for (size_t i = 0; i < single_scenes_.size(); i++) {
    if (selected_scene_ != i) {
      pcl::PointCloud<PointT>::Ptr cloud(new pcl::PointCloud<PointT>);
      pcl::transformPointCloud(*single_scenes_[i], *cloud, v4r::RotTrans2Mat4f(single_scenes_[i]->sensor_orientation_,
                                                                               single_scenes_[i]->sensor_origin_));
  ...
  if (!*okay) {
    std::cerr
        << "Could not convert icp iterations text field to a number. Is it a number? Setting it to the default value "
        << 20 << std::endl;
    icp_iter = 20;
  }
  ...
  if (selected_hypothesis_ >= 0) {
    ...
    Eigen::Matrix4f cloud_to_global = v4r::RotTrans2Mat4f(single_scenes_[selected_scene_]->sensor_orientation_,
                                                          single_scenes_[selected_scene_]->sensor_origin_);
...
void MainWindow::remove_selected() {
  std::cout << "Removed selected hypothesis " << selected_hypothesis_ << " from the " << sequence_hypotheses_.size()
            << " hypotheses present in total. " << std::endl;
...
void MainWindow::updateSelectedHypothesis() {
  ...
  if (selected_hypothesis_ >= 0) {
    ...
    pcl::PointCloud<PointT>::ConstPtr model_cloud =
        sequence_hypotheses_[selected_hypothesis_]->getAssembled(resolution_mm_);
    pcl::PointCloud<PointT>::Ptr model_cloud_transformed(new pcl::PointCloud<PointT>(*model_cloud));
    pcl::transformPointCloud(*model_cloud, *model_cloud_transformed, hypotheses_poses_[selected_hypothesis_]);
...
void MainWindow::x_plus() {
  ...
  if (!*okay) {
    std::cerr << "Could not convert translation step size. Is it a number? I set it to the default value "
              << translation_step_ << std::endl;
...
void MainWindow::xr_plus() {
  ...
  if (!*okay) {
    std::cerr << "Could not convert rotation step size. Is it a number? I set it to the default value "
              << translation_step_ << std::endl;
...
void MainWindow::save_model() {
  ...
  for (size_t i = 0; i < single_scenes_.size(); i++) {
    ...
    const Eigen::Matrix4f tf =
        v4r::RotTrans2Mat4f(single_scenes_[i]->sensor_orientation_, single_scenes_[i]->sensor_origin_);
    ...
    for (size_t k = 0; k < sequence_hypotheses_.size(); k++) {
      ...
      const Eigen::Matrix4f cloud_to_global =
          v4r::RotTrans2Mat4f(single_scenes_[i]->sensor_orientation_, single_scenes_[i]->sensor_origin_);
      ...
      occlusion_file << gt_or_ouput_dir << "/" << scene << "_occlusion_" << model_id_replaced << "_" << id_c_it->second
                     << ".txt";
...
MainWindow::MainWindow(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "produce help message")("models_dir,m", po::value<std::string>(&dir_models_)->required(),
                                                       "directory containing the model .pcd files")(
      "pcd_file,p", po::value<std::string>(&base_path_)->required(),
      "Directory with the to be annotated scenes stored as point clouds (.pcd). The camera pose is taken directly from "
      "the pcd header fields \"sensor_orientation_\" and \"sensor_origin_\" and it also looks for an initial "
      "annotation file \"results_3d.txt\". If this annotation file exists, the program reads each row as "
      "\"\\path\\to\\object_model.pcd t11 t12 t13 t14 t21 ...  t34 .. 0 0 0 1\" where t are elements of the 4x4 "
      "homogenous transformation matrix bringing the model into the world coordinate system. If the test directory "
      "contains subdirectories, each subdirectory is considered as seperate sequence for multiview recognition.")(
      "export_ground_truth_to,o",
      po::value<std::string>(&export_ground_truth_to_)->default_value("/tmp/exported_ground_truth/"),
      "Output directory")("icp_scene_to_model", po::value<bool>(&icp_scene_to_model_)->default_value(false),
                          "if true, does pose refinement.")(
      "model_scale", po::value<double>(&model_scale_)->default_value(model_scale_), "model scale")(
      "resolution_mm", po::value<int>(&resolution_mm_)->default_value(resolution_mm_),
      "Resolution in mm used for ICP and visualization");
  ...
  QObject::connect(model_list_, SIGNAL(clicked(const QModelIndex &)), this,
                   SLOT(model_list_clicked(const QModelIndex &)));
