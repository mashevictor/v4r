...
class V4R_EXPORTS ObjectRecognitionVisualizer {
 private:
  ...
  LocalObjectModelDatabase::ConstPtr
      lomdb_;  ///< pointer to local model database (optional: required if visualization of feature matching is desired)
 ...
 public:
   ...
   */
  void setGeneratedObjectHypotheses(const std::vector<ObjectHypothesesGroup<PointT>> &goh) {
   ...
   */
  void setVerifiedObjectHypotheses(const std::vector<typename ObjectHypothesis<PointT>::Ptr> &voh) {
   ...
   */
  void setLocalModelDatabase(LocalObjectModelDatabase::ConstPtr &lomdb) {
    lomdb_ = lomdb;
  }
