...
int main(int argc, char **argv) {
  ...
  int cg_thresh = 7;  // Threshold for correspondence grouping. The lower the more hypotheses are generated, the higher
  ...
  desc.add_options()("help,h", "produce help message")(
      ...
      "indices_fn_prefix", po::value<std::string>(&indices_fn_prefix)->default_value("object_indices_"),
      "Prefix of object indices filename")("transformation_fn",
                                           po::value<std::string>(&transformation_fn)->default_value(""),
                                           "Transformation to apply to each pose")(
      "pose_fn_prefix", po::value<std::string>(&pose_fn_prefix)->default_value("pose_"),
      "Prefix for the output pose filename")(
      ...
      "positives (noise increase with z)")("cg_thresh,c", po::value<int>(&cg_thresh)->default_value(cg_thresh),
                                           "Threshold for correspondence grouping. The lower the more hypotheses are "
                                           "generated, the higher the more confident and accurate. Minimum 3.")(
      "cg_size,g", po::value<float>(&cg_size)->default_value(cg_size, boost::str(boost::format("%.2e") % cg_size)),
      "Size for correspondence grouping.")("do_sift", po::value<bool>(&do_sift)->default_value(do_sift),
                                           "if true, enables SIFT feature matching")(
      ...
      "do_alexnet", po::value<bool>(&do_alexnet)->default_value(do_alexnet),
      "if true, enables AlexNet global matching")(
      "segmentation_method", po::value<int>(&segmentation_method)->default_value(segmentation_method),
      "segmentation method (as stated in the V4R library (modules segmentation/types.h) ")(
      "depth_img_mask", po::value<std::string>(&depth_img_mask)->default_value(depth_img_mask),
      "filename for image registration mask. This mask tells which pixels in the RGB image can have valid depth pixels "
      "and which ones are not seen due to the phsysical displacement between RGB and depth sensor.")
      //            ("hv_config_xml", po::value<std::string>(&hv_config_xml)->default_value(hv_config_xml), "Filename of
      //            Hypotheses Verification XML configuration file.")
      ("sift_config_xml", po::value<std::string>(&sift_config_xml)->default_value(sift_config_xml),
       "Filename of SIFT XML configuration file.")(
          "shot_config_xml", po::value<std::string>(&shot_config_xml)->default_value(shot_config_xml),
          "Filename of SHOT XML configuration file.")(
          "alexnet_config_xml", po::value<std::string>(&alexnet_config_xml)->default_value(alexnet_config_xml),
          "Filename of Alexnet XML configuration file.")(
          "esf_config_xml", po::value<std::string>(&esf_config_xml)->default_value(esf_config_xml),
          "Filename of ESF XML configuration file.")(
          "camera_xml", po::value<std::string>(&camera_config_xml)->default_value(camera_config_xml),
          "Filename of camera parameter XML file.")("visualize,v", po::bool_switch(&visualize),
          ...
          "dbg_dir", po::value<std::string>(&debug_dir)->default_value(debug_dir),
          "Output directory where debug information (generated object hypotheses) will be stored (skipped if empty)");
  po::variables_map vm;
    ...
    LocalRecognitionPipeline<PointT>::Ptr local_recognition_pipeline(new LocalRecognitionPipeline<PointT>);
    {
      ...
      if (do_sift || do_shot) {
        ...
        if (do_shot) {
          ...
          KeypointExtractor<PointT>::Ptr keypoint_extractor =
        ...
        RecognitionPipeline<PointT>::Ptr rec_pipeline_tmp =
            boost::static_pointer_cast<RecognitionPipeline<PointT>>(local_recognition_pipeline);
      ...
      if (do_esf || do_alexnet) {
        ...
        typename v4r::Segmenter<PointT>::Ptr segmenter =
            v4r::initSegmenter<PointT>(segmentation_method, to_pass_further);
  ...
  for (const std::string &sub_folder_name : sub_folder_names) {
    ...
    for (size_t v_id = 0; v_id < views.size(); v_id++) {
      ...
      if (visualize) {
        //                LocalObjectModelDatabase::ConstPtr lomdb =
        //                local_recognition_pipeline->getLocalObjectModelDatabase();
