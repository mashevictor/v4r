...
public:
  ...
  void useNoiseModel(bool enable) {use_noise = enable;}
  void filterLargestCluster(bool enable) { filter_largest_cluster = enable; }
...
private:
  ...
  void getMaskFromBasePlane(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose, cv::Mat_<unsigned char> &mask, std::vector<int> &indices);
