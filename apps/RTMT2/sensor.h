...
public:
  ...
  void showCameras(int enable) {m_show_cameras = enable;}
  void setTSFParameter(int nb_tracked_frames_ba, int batch_size_clouds_tsf, const double &cam_dist_map, const double &delta_angle_map);

  //void createModel();
  inline const std::vector<v4r::TSFFrame::Ptr> &getMap() const { return tsf.getMap(); }
...
private:
  ...
  void maskCloud(const Eigen::Matrix4f &pose, const Eigen::Vector3f &bb_min, const Eigen::Vector3f &bb_max, pcl::PointCloud<pcl::PointXYZRGB> &_cloud);
