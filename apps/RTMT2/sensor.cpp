...
Sensor::Sensor() :
  ...
  bbox_scale_height(1.),
  seg_offs(0.01),
  bb_min(Eigen::Vector3f(-FLT_MAX,-FLT_MAX,-FLT_MAX)),
 ...
 */
void Sensor::setTSFParameter(int nb_tracked_frames_ba, int batch_size_clouds_tsf, const double &cam_dist_map, const double &delta_angle_map)
{
...
void Sensor::run()
{
  ...
  while(m_run)
  {
    ...
    if (cloud.get()!=0)
    {
      ...
      if (m_activate_roi) maskCloud(pose*bbox_base_transform, bb_min, bb_max, *cloud);
...
//        {
//            pcl::io::savePCDFileBinary(std::string("log/")+v4r::toString(z)+std::string("-filt.pcd"), filt_cloud);
...
//        }
        if (have_pose)
        {
          ...
          if (m_show_cameras) emit update_cam_trajectory(cam_trajectory);
        }
 ...
 * @param bb_max
 */
void Sensor::maskCloud(const Eigen::Matrix4f &_pose, const Eigen::Vector3f &_bb_min, const Eigen::Vector3f &_bb_max, pcl::PointCloud<pcl::PointXYZRGB> &_cloud)
{
