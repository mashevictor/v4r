...
static bool parseArgs(int argc, char** argv) {
  ...
  forest.add_options()("help,h", "")("forest,f", po::value<std::string>(&forestfile)->default_value("forest.ef"),
                                     "Stored forest file")(
  ...
  po::variables_map vm;
  po::store(po::command_line_parser(argc, argv).options(forest).run(), vm);
...
static void CreateTopNHistograms(int nrOfLabels,
                                 std::vector<std::vector<v4r::EntangledForestTopNFeature*>>& pairwisetopn,
                                 std::vector<std::vector<v4r::EntangledForestInverseTopNFeature*>>& inversetopn,
                                 std::vector<std::vector<unsigned int>>& pairwisetopnhisthor,
                                 std::vector<std::vector<unsigned int>>& pairwisetopnhistver,
                                 std::vector<std::vector<unsigned int>>& inversetopnhist) {
...
static void CreateAngleAndDistLists(
    std::vector<std::vector<v4r::EntangledForestTopNFeature*>>& pairwisetopn,
    std::vector<std::vector<v4r::EntangledForestInverseTopNFeature*>>& inversetopn,
    std::vector<std::vector<double>>& meananglehor, std::vector<std::vector<double>>& meananglever,
    std::vector<std::vector<double>>& meanangleinv, std::vector<std::vector<double>>& meandisthor,
    std::vector<std::vector<double>>& meandistver, std::vector<std::vector<double>>& meandistinv,
    std::vector<std::vector<double>>& anglecorrhor, std::vector<std::vector<double>>& anglecorrver,
    std::vector<std::vector<double>>& anglecorrinv, std::vector<std::vector<double>>& distcorrhor,
    std::vector<std::vector<double>>& distcorrver, std::vector<std::vector<double>>& distcorrinv) {
  meananglehor.clear();
  meananglever.clear();
  meanangleinv.clear();
  meandisthor.clear();
    ...
    for (int n = 0; n < tree->GetNrOfNodes(); ++n) {
      ...
      if (node->IsSplitNode()) {
        ...
        for (unsigned int l = 0; l < parentdist.size(); ++l) {
          //                    double parentbin =
          //                    -log2(parentdist[l]+std::numeric_limits<double>::min())*parentdist[l];
        ...
        l2l << setw(2) << depth << " " << feattype << " " << setw(2) << targetlbl << " " << setw(2) << splitlbl << " "
            << angle << " " << diff << std::endl;
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  CreateAngleAndDistLists(topnfeatures, itopnfeatures, meanangleshor, meananglesver, meananglesinv, meandisthor,
                          meandistver, meandistinv, anglecorrhor, anglecorrver, anglecorrinv, distcorrhor, distcorrver,
                          distcorrinv);
  ...
  for (int tidx = 0; tidx < f.GetNrOfTrees(); ++tidx) {
    ...
    for (int nidx = 0; nidx < t->GetNrOfNodes(); ++nidx) {
      v4r::EntangledForestNode* n = t->GetNode(nidx);
      int d = n->GetDepth();
      std::transform(bla[d].begin(), bla[d].end(), n->GetLabelDistribution().begin(), bla[d].begin(),
                     std::plus<double>());
    }
  }
